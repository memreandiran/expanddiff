...
def clip_soft_mask(guidance, thr=0.05):
    ...
    lo = ((thr - g.amin(dim=1, keepdim=True)) / thr).clamp(0.0, 1.0)
    return th.maximum(hi, lo)


def _masked_mean_flat(d, w):
    """mean over all but the batch dim, weighted by w (broadcast over channels)."""
    num = mean_flat(d * w)
    den = mean_flat(w.expand_as(d)).clamp(min=1e-6)
    return num / den
from .losses import normal_kl, discretized_gaussian_log_likelihood, logl1_loss, pul1_loss
    ...
    def training_losses(
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if clip_loss_weight > 0.0:
                ...
                if _g is not None:
                    _m = clip_soft_mask(_g, clip_loss_thr).to(target.dtype)
                    _sw = (1.0 - clip_loss_weight) + clip_loss_weight * _m
                    terms["clip_mask_frac"] = mean_flat(_m)

            def _red(d):
                return _masked_mean_flat(d, _sw) if _sw is not None else mean_flat(d)

            if weight_l2 > 0.0:
                mse = weight * _red((target - model_output) ** 2)
                terms["mse"] = mse
                rec_loss += weight_l2 * mse

            if weight_l1 > 0.0:
                l1 = weight * _red(th.abs(target - model_output))
                terms["l1"] = l1
            ...
            if weight_pul1 > 0.0:
                pul1 = weight * pul1_loss(model_output_normalized, target_normalized)
                terms["pul1"] = pul1
                rec_loss += weight_pul1 * terms["pul1"]

            extra["target"] = target
            extra["model_output"] = model_output

            if "vb" in terms:
                terms["loss"] = rec_loss + terms["vb"]
            else:
                terms["loss"] = rec_loss
        else:
